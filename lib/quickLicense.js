...
class QuickLicense {
  ...
  async glob(pattern, cwd, ignore=[]) {
    return new Promise((resolve, reject) => {
      glob(pattern, { nodir: true, cwd: cwd, ignore:ignore }, (er, files) => {
  ...
  async getFiles() {
    ...
    for (const pattern of this.config.include) {
      console.log("Working on pattern:", pattern);
      files = files.concat(await this.glob(pattern, process.cwd(), this.config.exclude));
    }
